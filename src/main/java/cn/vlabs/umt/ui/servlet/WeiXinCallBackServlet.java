...
public class WeiXinCallBackServlet extends HttpServlet {
	private static final Logger LOGGER = Logger.getLogger(WeiXinCallBackServlet.class);
	@Override
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException,
			IOException {
		try {
			WeiXinAccessToken accessTokenObj=WeiXinUrils.getAccessToken(request);
			if (CommonUtils.isNull(accessTokenObj.getAccessToken())){
				...
				SessionUtils.setSessionVar(request, Attributes.THIRDPARTY_TYPE, BindInfo.TYPE_WEIXIN);
				UserService userService=ServiceFactory.getUserService(request);
				cn.vlabs.umt.services.user.bean.User umtUser=userService.getUserByOpenid(openID,BindInfo.TYPE_WEIXIN,null);
				if(umtUser==null){
					response.sendRedirect(RequestUtil.getContextPath(request)+"/accountBind_weixin_umt.jsp");
				}else{
					response.sendRedirect(RequestUtil.getContextPath(request)+"/login?type=weixin&act=Validate&authBy=weixin"+getSiteInfoParam(request));
				}
			}
