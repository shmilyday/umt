...
<div class="content-container">
	<div class="toolBar">
		<form id="formId" class="form-search" style="float: right; margin: 0"
			method="post" action="<umt:url value="/admin/accessIps.do?act=add"/>">
			IP:<input name="ip" id="ip" value="" type="text" />
			<input name="scope" type="radio" value="A" /> A
			<input name="scope" type="radio" value="B" checked/> B
			<fmt:message key="admin.access.ip.remark" />:<input name="remark" id="remark " value="" type="text" />
		...
		<table class="table table-hover" id="sessions" border="0">
			<tbody>
				<tr class="top">
					<td>IP</td>
					<td>Scope</td>
					<td><fmt:message key="admin.access.ip.remark" /></td>
				...
				<c:forEach items="${ips }" var="ip">
					<tr class="firstline ">
						...
						<td>${ip.remark }</td>
						<td><a href="#" onclick="deleteIp('${ip.id}')"><fmt:message
