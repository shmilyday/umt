...
public class DoActivationServiceForPrimary extends AbstractDoActivation{
	...
	public DoActivationServiceForPrimary(HttpServletRequest request, HttpServletResponse response, Token token,
			User user, ActivationForm data,LoginNameInfo nameInfo,ActionMapping mapping) {
	...
	public ActionForward hasLoginAndIsSelf() throws Exception {
		...
		if(getData().isChangeLoginName()){
			...
			if(getUserService().isUsed(nameInfo.getLoginName())){
				return toError();
			}else{
	...
	public ActionForward hasLoginAndNotSelf() throws Exception {
		String rtnUrl=URLEncoder.encode(getPrimaryLoginUrlOnlyActive(getData(),getRequest(), getUser().getCstnetId(),"activeLogin"),"UTF-8");
	...
	public ActionForward notLogin() throws Exception {
		getResponse().sendRedirect(getPrimaryLoginUrlOnlyActive(getData(),getRequest(),getUser().getCstnetId(), "activeLogin"));
		return null;
	}
