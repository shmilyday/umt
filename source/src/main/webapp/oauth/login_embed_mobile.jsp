...
<body class="login oauth embed_mobile">
	<jsp:include flush="true" page="../banner2013CSSJSOnly.jsp"></jsp:include>
			<f:script  src="${context }/js/jquery.tmpl.min.js"/>
	<f:script src="${context }/js/ValidateCode.js"/>
	<f:css href="${context }/css/oauth.css"/>
	<f:css href="${context }/css/embed_mobile.css"/>
	<div class="container login">
		<form id="loginForm" action="authorize?${user_oauth_request.url}" method="post" class="form-horizontal oauth">
			...
			<div class="input-append">
            	...
            	<input tabindex=1 type="text" id="userName" name="userName" placeholder="中国科技网通行证" value="${!empty userName?userName:autoFill }"/>
		    ...
		    <div class="input-append down" id="mainDiv">
				<span class="add-on" id="${preSpanId }"></span>
            	<input tabindex=2 type="password" id="password" name="password" placeholder="密码" value="${password }">
            ...
            <input tabindex=3 type="submit" class="btn long btn-primary" style="background-image:none;text-shadow:none;filter:none; box-shadow:none; border-color:#006dcc" value="<fmt:message key='common.banner.login'/>">
            <%-- <p class="hintMe">
            	<umt:registerLink> <fmt:message key='login.regist'/></umt:registerLink>
            	<a class="forget" href="<umt:config key="umt.this.base.url"/>/findPsw.do?act=stepOne"><fmt:message key='login.forgetpassword'/></a>
            </p> --%>
