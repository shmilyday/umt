...
<table class="table accountManage">
	<thead>
		<tr>
			...
			<th style="width: 10em"><fmt:message
					key='requests.list.username'></fmt:message></th>
		...
		<tr>
			...
			<td class="left"><c:choose>
					<c:when test="${!empty nameInfo }">
						<c:out value="${nameInfo.loginName }" />
					</c:when>
			...
			<td><c:choose>
					<c:when test="${ empty app.secret}">
						<a class=" btn btn-small btn-info"
							href="<umt:url value="/user/manage.do?act=createPassword&viewType=ldap&appId=${ app.appId}"/>"><fmt:message
								key='app.access.pwd.open' /></a>
					</c:when>
					<c:otherwise>
						<a data-pwd-id="${app.secret.id }"
							data-app-id="${app.secret.appId }" data-view-type="ldap"
							class=" btn btn-small btn-info deletePwd"><fmt:message
								key='app.access.pwd.close' /></a>
						<a
							href="<umt:url value="/user/manage.do?act=createPassword&viewType=ldap&appId=${ app.appId}&op=updatePwd"/>"><fmt:message
